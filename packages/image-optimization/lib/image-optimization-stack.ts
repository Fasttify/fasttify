...
// CloudFront parameters - Integración con CloudFront existente
var EXISTING_CLOUDFRONT_DISTRIBUTION_ID: string;
...
export class ImageOptimizationStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    S3_TRANSFORMED_IMAGE_EXPIRATION_DURATION =
      this.node.tryGetContext('S3_TRANSFORMED_IMAGE_EXPIRATION_DURATION') || S3_TRANSFORMED_IMAGE_EXPIRATION_DURATION;
    S3_TRANSFORMED_IMAGE_CACHE_TTL =
      this.node.tryGetContext('S3_TRANSFORMED_IMAGE_CACHE_TTL') || S3_TRANSFORMED_IMAGE_CACHE_TTL;
    ...
    EXISTING_CLOUDFRONT_DISTRIBUTION_ID =
      this.node.tryGetContext('EXISTING_CLOUDFRONT_DISTRIBUTION_ID') || EXISTING_CLOUDFRONT_DISTRIBUTION_ID;
    CLOUDFRONT_ORIGIN_SHIELD_REGION =
      this.node.tryGetContext('CLOUDFRONT_ORIGIN_SHIELD_REGION') || CLOUDFRONT_ORIGIN_SHIELD_REGION;
    ...
    // Integración con CloudFront existente en lugar de crear uno nuevo
    if (EXISTING_CLOUDFRONT_DISTRIBUTION_ID) {
      // Referenciar la distribución de CloudFront existente
      const existingDistribution = cloudfront.CfnDistribution.fromDistributionId(
        this,
        'existing-cloudfront-distribution',
        EXISTING_CLOUDFRONT_DISTRIBUTION_ID
      );

      // Crear OAC para Lambda
      ...
      // Nota: La asociación de funciones a distribuciones existentes debe hacerse manualmente
      // ya que CDK no puede modificar distribuciones creadas fuera del stack

      // Permisos para CloudFront existente invocar Lambda
      imageProcessing.addPermission('AllowCloudFrontServicePrincipal', {
        principal: new iam.ServicePrincipal('cloudfront.amazonaws.com'),
        action: 'lambda:InvokeFunctionUrl',
        sourceArn: `arn:aws:cloudfront::${this.account}:distribution/${EXISTING_CLOUDFRONT_DISTRIBUTION_ID}`,
      });

      new CfnOutput(this, 'ManualIntegrationSteps', {
        description: 'Pasos para integrar manualmente con CloudFront',
        value: `
Para completar la integración con tu CloudFront distribution ${EXISTING_CLOUDFRONT_DISTRIBUTION_ID}:

1. Ve a la consola de CloudFront
2. Selecciona tu distribución ${EXISTING_CLOUDFRONT_DISTRIBUTION_ID}
3. Ve a "Behaviors" y edita el "Default Cache Behavior"
4. En "Function associations":
   - Event type: Viewer Request
   - Function type: CloudFront Function
   - Function ARN: ${urlRewriteFunction.functionArn}

Después podrás usar parámetros de optimización:
- ?width=300
- ?format=jpeg
- ?format=auto&quality=80
- ?format=avif&width=400
        `,
      });
    } else {
      // Fallback: crear nueva distribución si no se especifica una existente
      ...
      cfnImageDelivery.addPropertyOverride(
        `DistributionConfig.Origins.${STORE_TRANSFORMED_IMAGES === 'true' ? '1' : '0'}.OriginAccessControlId`,
        oac.getAtt('Id')
      );
