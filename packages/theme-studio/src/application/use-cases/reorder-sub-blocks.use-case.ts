...
import type { IDevServer, ReorderSubBlocksParams } from '../../domain/ports/dev-server.port';
...
/**
 * Caso de uso: Reordenar sub-bloques dentro de un bloque
 * Orquesta la lógica de negocio para reordenar sub-bloques y notificar al Dev Server
 */
export class ReorderSubBlocksUseCase {
  constructor(
  ...
  async execute(params: ReorderSubBlocksParams): Promise<void> {
    ...
    // 3. Validar que la sección y el bloque existen
    const section = templateBefore.sections[params.sectionId];
    if (!section) {
      throw new Error('Section not found');
    }

    const block = section.blocks?.find((b) => b.id === params.blockId);
    if (!block) {
      throw new Error('Block not found');
    }

    if (!block.blocks || block.blocks.length === 0) {
      throw new Error('Block has no sub-blocks');
    }
    ...
    if (params.oldIndex >= block.blocks.length || params.newIndex >= block.blocks.length) {
    ...
    const change = {
      ...
      type: 'REORDER_SUB_BLOCKS' as ChangeType,
      sectionId: params.sectionId,
      blockId: params.blockId,
      subBlockId: block.blocks[params.oldIndex].id,
    ...
    await this.devServer.reorderSubBlocks(params);
  }
}
