 ...
 */
type MenuProcessor = (menu: ProcessedNavigationMenu, config: MenuProcessingConfig) => LinkList[];
...
export class LinkListService {
  ...
  public async createLinkListsFromDatabase(
    ...
    try {
      ...
      logger.error(`Error creating linklists from database for store ${storeId}`, error, 'LinkListService');
      return {};
    }
  }
  ...
  private getMenuProcessor(menu: ProcessedNavigationMenu): MenuProcessor {
    ...
    const hasNestedItems = menu.items.some((item) => (item as any).children && (item as any).children.length > 0);
   ...
   */
  public createLinkListsFromTemplate(templateConfig: any, customConfig?: Partial<MenuProcessingConfig>): LinkLists {
    const linkLists: LinkLists = {};
    const effectiveConfig = customConfig ? { ...this.config, ...customConfig } : this.config;
  ...
  private extractMenusFromTemplate(templateConfig: any): Array<{
    ...
    if (templateConfig?.menus) {
      for (const [handle, menuData] of Object.entries(templateConfig.menus)) {
        ...
        menus.push({
          handle,
          title: menu.title || handle.replace('-', ' ').replace(/\b\w/g, (l: string) => l.toUpperCase()),
          items: menu.items || [],
        });
      }
    }
 ...
 */
function convertMenuToLinkList(menu: ProcessedNavigationMenu, config: MenuProcessingConfig): LinkList {
