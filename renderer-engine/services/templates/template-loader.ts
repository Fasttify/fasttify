...
class TemplateLoader {
    ...
    try {
      ...
      if (this.isProduction && this.cloudFrontDomain) {
        const response = await fetch(
          `https://${this.cloudFrontDomain}/templates/${storeId}/${templatePath}`
        )
        if (!response.ok) {
  ...
  public async loadSection(storeId: string, sectionName: string): Promise<string> {
    const fileName = sectionName.endsWith('.liquid')
      ? sectionName
      : `${sectionName}.liquid`
  ...
  public async loadAsset(storeId: string, assetPath: string): Promise<Buffer> {
    ...
    try {
      if (this.isProduction && this.cloudFrontDomain) {
        const response = await fetch(
          `https://${this.cloudFrontDomain}/templates/${storeId}/assets/${assetPath}`
        )
        if (!response.ok) {
