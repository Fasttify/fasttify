...
async function loadDataStep(data: RenderingData): Promise<RenderingData> {
  logger.info(
    `Using dynamic data loading for ${data.options.pageType}`,
    'DynamicPageRenderer'
  )
  ...
  const [layout, pageData, storeTemplate, pageTemplate] = await Promise.all([
    ...
    dynamicDataLoader.loadDynamicData(
      data.store!.storeId,
      data.options,
      data.searchParams
    ),
...
async function buildContextStep(data: RenderingData): Promise<RenderingData> {
  ...
  ;(context as any).request = {
    searchParams: new URLSearchParams(
      Object.entries(data.searchParams).map(([key, value]) => [key, value])
    ),
  }
...
async function renderLayoutStep(data: RenderingData): Promise<RenderingData> {
  ...
  ;(data.context as any).content_for_header = metadataGenerator.generateHeadContent(
    data.store!
  )
 ...
 */
async function preloadLayoutSections(
  storeId: string,
  layout: string,
  context: any,
  storeTemplate: any
