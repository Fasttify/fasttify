...
describe('SecureLogger', () => {
  ...
  describe('Format String Protection', () => {
    ...
    test('should handle multiple malicious inputs', () => {
      ...
      expect(mockConsole.warn).toHaveBeenCalledWith('Multiple inputs: %%s %%s', '%%d%%s%%x', '\\r\\n\\t');
    });
    ...
    test('should handle non-string inputs safely', () => {
      ...
      expect(mockConsole.debug).toHaveBeenCalledWith('Mixed inputs: %%s %%s %%s', 123, { key: 'value%d' }, null);
    });
  });
  ...
  describe('Real-world Attack Scenarios', () => {
    test('should prevent CloudFront domain injection', () => {
      ...
      expect(mockConsole.error).toHaveBeenCalledWith('Error creating tenant for domain %s:', 'evil.com%%d%%s%%x');
    });
    ...
    test('should prevent user ID injection in store operations', () => {
      ...
      expect(mockConsole.log).toHaveBeenCalledWith('Store operation for user %s:', 'user123%%d%%s\\r\\n\\t');
    });
    ...
    test('should prevent template path injection', () => {
      ...
      expect(mockConsole.warn).toHaveBeenCalledWith('Template not found: %s', '/templates/index.liquid%%d\\0');
    });
  });
