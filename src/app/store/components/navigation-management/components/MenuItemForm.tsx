...
import { DraggableMenuItems } from '@/app/store/components/navigation-management/components/DraggableMenuItems';
...
import { Button, Checkbox, Collapsible, Select, Text, TextField } from '@shopify/polaris';
    ...
    <DraggableMenuItems
      ...
      {(item) => (
        <div>
          <div className="flex items-center justify-between w-full">
            ...
            <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
              ...
              <Button
                size="micro"
                variant="tertiary"
                onClick={() => setEditingIndex(editingIndex === item.originalIndex ? null : item.originalIndex)}
                icon={EditIcon}
                accessibilityLabel={editingIndex === item.originalIndex ? 'Cerrar edición' : `Editar ${item.label}`}
                disabled={disabled}
              />
            </div>
          </div>
          ...
          <Collapsible
            open={editingIndex === item.originalIndex}
            id={`collapsible-${item.originalIndex}`}
            transition={{
              duration: '200ms',
