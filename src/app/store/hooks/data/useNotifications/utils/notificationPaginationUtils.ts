...
/**
 * Hook personalizado para manejar la paginación de notificaciones
 */
export const useNotificationPagination = (options: PaginationOptions) => {
  const [currentPage, setCurrentPage] = useState(1);
  const [pageTokens, setPageTokens] = useState<(string | null)[]>([null]);

  const limit = options.limit || 50;
  const sortDirection = options.sortDirection || 'DESC';
  const sortField = (options.sortField as string) || 'createdAt'; // Default a createdAt
  ...
  const hasNextPage = useCallback(
    (nextToken: string | null, itemsCount: number = 0) => {
      // Si no hay nextToken, no hay más páginas
      if (!nextToken) return false;
      // Si tenemos menos items que el límite, no hay más páginas
