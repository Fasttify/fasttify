...
const MenuItem: React.FC<MenuItemProps> = ({ link, text, image }) => {
  ...
  const repeatedMarqueeContent = React.useMemo(() => {
    return Array.from({ length: 4 }).map((_, idx) => (
      <React.Fragment key={idx}>
        <span className="text-white uppercase font-medium text-[4vh] leading-[1.2] p-[1vh_1vw_0]">{text}</span>
        <div
  ...
  return (
    <div className="flex-1 relative overflow-hidden text-center shadow-[0_-1px_0_0_#e0e0e0]" ref={itemRef}>
      <a
