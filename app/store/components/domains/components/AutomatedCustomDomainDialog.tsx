...
export function AutomatedCustomDomainDialog({
  ...
  const handleValidateACM = async () => {
    ...
    if (result.success && result.data?.isReady) {
      ...
      showToast(result.error ?? 'El certificado SSL aún no está listo. Inténtalo de nuevo en unos minutos.', true);
    }
  };
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      ...
      case 'cloudfront':
        return <LoadingStep title="Conectando con tu tienda" description="Esto puede tomar unos minutos..." />;
      case 'complete':
        return <CompletionStep domain={domainName} method={validationMethod ?? 'cloudfront'} onClose={handleClose} />;
      default:
        return (
          <DomainInputStep
            domainName={domainName}
  ...
  return (
    <>
      ...
      <Modal
        ...
        <Modal.Section>
          <p>
            Si sales ahora, perderás todo el progreso de configuración del dominio personalizado. Tendrás que empezar el
