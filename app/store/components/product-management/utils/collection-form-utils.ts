...
export const useCollectionForm = ({
  ...
  // Sincronizar el slug con el título, permitiendo slugs personalizados.
  useEffect(() => {
    ...
    // Para nuevas colecciones, el slug siempre se sincroniza con el título.
    if (!isEditing) {
      setSlug(generateSlug(title));
      return;
    }

    // Para colecciones existentes, solo se sincroniza si el slug no fue editado manualmente.
    // Se infiere que fue manual si el slug inicial no coincide con el que se generaría del título inicial.
    if (isDataLoaded) {
      const wasSlugAutoGenerated = initialFormState.slug === generateSlug(initialFormState.title);
      if (wasSlugAutoGenerated) {
        setSlug(generateSlug(title));
      }
    }
  }, [title, isEditing, isDataLoaded, initialFormState.slug, initialFormState.title]);
