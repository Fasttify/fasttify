...
export function ProductSection({ selectedProducts = [], onAddProduct, onRemoveProduct }: ProductSectionProps) {
  const [itemsPerPage, setItemsPerPage] = useState(50);
  const { storeId } = useStoreDataStore();
  const fetchTriggered = useRef(false);

  const { products, loading, refreshProducts, hasNextPage, hasPreviousPage, nextPage, previousPage, currentPage } =
